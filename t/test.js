...
{	
	...
	{
		fname_s_0265({type: 'wd', wd: obj, lotid: obj.userData.door.lotid});
	}
...
function fname_s_057(cdm)
{
	...
	{  
		...
		else if( tag == 'substrate_tool' && camera == cameraTop ) { fname_s_0304({intersect: rayhit}); }
		else if( tag == 'pivot' ) { fname_s_0242( rayhit ); }
		else if( tag == 'gizmo' ) { fname_s_0257( rayhit ); } 
		...
		else if( tag == 'obj' && camera == cameraTop ) { fname_s_0248( obj ); }		
		else { flag = false; }
	}
	else if(cdm.type == 'up')
	{	
		if( tag == 'wall' && camera == camera3D ) {  }
		else if( tag == 'obj' && camera == camera3D ) { fname_s_0248( obj ); }
		else if( tag == 'room' && camera == cameraTop ) { clickO.last_obj = obj; fname_s_0277({obj: obj}); }
		else { flag = false; }
	}	
...
{ 
	...
	if ( obj ) 
	{
		...
		if ( tag == 'substrate_tool' ) { fname_s_0305(event); }	
		else if ( tag == 'pivot' ) { fname_s_0243( event ); }
		else if ( tag == 'gizmo' ) { fname_s_0259( event ); }
		...
		else if ( tag == 'obj' ) { fname_s_0246( event ); }
	}
	else 
	{
...
function fname_s_059( event )  
{
	...
	if(obj)  
	{
		...
		else if(tag == 'obj') { fname_s_0247(obj); }
...
function fname_s_060(cdm)
{
	...
	{ 
		...
		if(cdm.type == 'down')
		{
			...
			else if(tag == 'obj' && camera == cameraTop) { fname_s_0250(obj); }
			else { flag = false; }
		}
		else if(cdm.type == 'up')
		{
			if(tag == 'room' && camera == cameraTop) { fname_s_061(obj); }
			else if(tag == 'obj' && camera == camera3D) { fname_s_0250(obj); }
			else { flag = false; }
		}
		else
		{
...
{ 
	if ( camera3D.userData.camera.type == 'fly' )
	{
		...
		{  
			...
			if(gizmo.visible) fname_s_0256(gizmo.userData.gizmo.obj);
...
function fname_s_0129()
{
	...
	else if ( tag == 'obj' ) { fname_s_0249(obj); }
	...
	if(cdm.id)
	{
		...
		({
			...
			{ 			
				...
				if(cdm.upUI) { fname_s_0284({id: infProject.user.id}); }		
			},
			error: function(json){  }
		});			
	}
...
function fname_s_0191(cdm)
{
	...
	{
		fname_s_0265({lotid: lotid[i], loadFromFile: true, furn: furn});
	}	
}
...
infProject.scene.substrate.ruler = fname_s_0291(); 
...
infProject.tools = { pivot: fname_s_0239(), gizmo: fname_s_0255(), cutWall: [], point: fname_s_0211(), axis: fname_s_0207(), controllWD: fname_s_085() } 

infProject.catalog = { obj: fname_s_0262(), texture: fname_s_0263() }; 
...
{
	fname_s_0201();
	fname_s_0294({ pos: {y: 0.01} }); 	
	fname_s_070({radious: 15, theta: 90, phi: 35});		
	fname_s_0270();			
	fname_s_0271();		
	fname_s_0272();
	fname_s_0276({name: 'button_wrap_object'});	
...
function fname_s_0217( event )
{
	...
	{ 
		...
		{
			fname_s_0265({lotid: clickO.options, cursor: true});
		}		
	}
	else if(camera == camera3D)
	{
...
$('[nameId="button_load_1"]').mousedown(function () { fname_s_0232({value: 'button_load_1'}); });
$('[nameId="button_save_1"]').mousedown(function () { fname_s_0232({value: 'button_save_1'}); });
...
$('[nameId="select_pivot"]').mousedown(function () { fname_s_0251({mode:'pivot'}); });
$('[nameId="select_gizmo"]').mousedown(function () { fname_s_0251({mode:'gizmo'}); });

$('[nameId="obj_rotate_reset"]').mousedown(function () { fname_s_0254(); });	
$('[nameId="button_copy_obj"]').mousedown(function () { fname_s_0253(); });
$('[nameId="button_delete_obj"]').mousedown(function () { fname_s_0249(clickO.last_obj); });
...
$('input').on('focus', function () { fname_s_0233({el: $(this), act: 'down'}); });
$('input').on('change', function () { fname_s_0233({el: $(this), act: 'up'}); });
...
$('[nameId="button_check_reg_1"]').mousedown(function () { fname_s_0235({el: this}); });
$('[nameId="button_check_reg_2"]').mousedown(function () { fname_s_0235({el: this}); });	
...
$('[nameId="act_reg_1"]').mousedown(function () { fname_s_0236(); });
			...
			{  
				if(type=='reg_1')					{
					if(data.success)
					{
						...
						fname_s_0284({id: infProject.user.id});
					}
					else
					{
						if(data.err.desc)
...
$('[nameId="button_show_panel_catalog"]').mousedown(function () { fname_s_0237({show: true}); });
$('[nameId="button_catalog_close"]').mousedown(function () { fname_s_0237({show: false}); });
...
$('#load_substrate_1').change(fname_s_0238);	
$('[nameId="assign_size_substrate"]').mousedown(function () { fname_s_0311(); });
$('[nameId="button_delete_substrate"]').mousedown(function () { fname_s_0314(); }); 

$('[nameId="input_rotate_substrate_45"]').mousedown(function () { fname_s_0312({angle: 45}); });
$('[nameId="input_rotate_substrate_90"]').mousedown(function () { fname_s_0312({angle: 90}); });


$('[nameId="input_transparency_substrate"]').on("input", function() { fname_s_0313({value: $(this).val()}); }); 


function fname_s_0238(e) 
{
	if (this.files[0]) 
	{		
		if (this.files[0].type == "image/png" || this.files[0].type == "image/jpeg")
		{
			...
			reader.onload = function (e) 
			{
				...
				fname_s_0302({image: e.target.result});					
			}				
...
function fname_s_0239()
{
	...
	{
		var geometry = fname_s_0240(param[i].size_1.x, param[i].size_1.y, param[i].size_1.z);
		...
		if(param[i].size_2)
		{
			var axis = new THREE.Mesh( fname_s_0240(0.6, 0.02, 0.02), new THREE.MeshPhongMaterial({ color: param[i].color, depthTest: false, transparent: true, lightMap: lightMap_1 }) );	
			axis.renderOrder = 2;
	...
	pivot.add( fname_s_0241({axis: 'z', pos: new THREE.Vector3(0,0,-0.6), rot: new THREE.Vector3(-Math.PI/2,0,0), color: 0x0000ff}) );
	pivot.add( fname_s_0241({axis: 'x', pos: new THREE.Vector3(0.6,0,0), rot: new THREE.Vector3(0,0,-Math.PI/2), color: 0xff0000}) );
	pivot.add( fname_s_0241({axis: 'y', pos: new THREE.Vector3(0,0.6,0), rot: new THREE.Vector3(0,0,0), color: 0x00ff00}) );
...
function fname_s_0242( intersect )
{
...
function fname_s_0245( obj, intersect )
{	
	var obj = clickO.move = intersect.object;  
...
function fname_s_0257( intersect )
{			
	var gizmo = infProject.tools.gizmo;
	...
	{
		fname_s_0258(obj, dr, rotY, false);
	}
	
	
	function fname_s_0258(obj, dr, rotY, global)
	{
		if(global)	
		{
...
{	
	...
	{ 		
		fname_s_0260({obj: [obj], dr: dr, rotY: rotY, centerO: obj});		 
	}		
...
{ 
	...
	var inf = fname_s_0264({lotid: lotid});

	if(!inf) return;		
	var obj = fname_s_0266({lotid: lotid});
	...
	else
	{
		...
		{ 
			...
			var obj = fname_s_0267({lotid: lotid, obj: obj});
...
function fname_s_0268(inf, cdm)
{
	...
	if(inf.type)
	{
		if(inf.type == 'light point')
		{
			...
			fname_s_0269({obj: obj, intensity: intensity}); 
		}
	}		
...
{  
	...
	({
		...
		{  
			...
			$('[nameId="save_pr_1"]').on('mousedown', function(){ fname_s_0285(this); });
			$('[nameId="load_pr_1"]').on('mousedown', function(){ fname_s_0286(this); });
		}
	});	
}


function fname_s_0285(el)
{
...
function fname_s_0316(cdm)
{
	...
	for( var i = 0; i < rooms.length; i++ )
	{
		...
		{
			var x = w[i2].inner_part.point_1.x/100 * 1;
			var z = w[i2].inner_part.point_1.y/100 * 1;
	...
	for( var i = 0; i < arr.length; i++ )
	{
		for( var i2 = 0; i2 < arr[i].w.length; i2++ )
		{
			...
			var obj = fname_s_0214({p: [point1, point2], width: 0.2}); 
		}
	}
